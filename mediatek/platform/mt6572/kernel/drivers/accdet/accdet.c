...
//BEGIN: FR803757 Modified by fangjie, refer to soul45 longfang 
//static int micbias_times_flag =10; //6s*10=1min;
//END: FR803757 Modified by fangjie, refer to soul45 longfang 
...
if(debug_enable) \
{\
printk(KERN_ERR "[LLF--------------------Accdet]" format,##args);\
}\
}while(0)
//END: FR803757 Modified by fangjie, refer to soul45 longfang 
...
/*Begin ersen.shang 20150818 remove the modifications reference pr851125 for mid_key issue*/
#if 0 
//BEGIN: add by fangjie for PR851790, refer to PR851125
static char mic_press_voltage=0;//add by llf for PR851125
#define MIC_VOLTAGE18	18
//END: add by fangjie for PR851790, refer to PR851125
#endif
/*End   ersen.shang 20150818 remove the modifications reference pr851125 for mid_key issue*/
...
void accdet_detect(void)
{
	int ret = 0 ;
	//BEGIN: FR803757 Modified by fangjie, refer to soul45 longfang 
	ACCDET_DEBUG("[Accdet]accdet_detect works after -----5 seconds----- and check again!\n");
	//END: FR803757 Modified by fangjie, refer to soul45 longfang 
...
static void disable_micbias(unsigned long a)
{
	//BEGIN: FR803757 Modified by fangjie, refer to soul45 longfang 
	int ret = 0;
	//dump_register();//add by llf;
	printk(" [Audio_Accdet] The 6s timer on works#############\n");
	ret = queue_work(accdet_workqueue, &accdet_work);	
	if(!ret)
	{
		ACCDET_DEBUG("[PMIC_MIC_INT][Accdet]accdet_work return:%d!\n", ret);  		
	}
	printk(" [Audio_Accdet] ==================After 6s  check again!================\n");
	mdelay(100);//give time to check cable type;
	...
	/*
	if(micbias_times_flag){
	micbias_times_flag--;//check time for 1min;
...
static void disable_micbias_callback(struct work_struct *work)
{
	//BEGIN: FR803757 Modified by fangjie, refer to soul45 longfang 
	//int ret=0;
	//ret=((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0)>>6); //A=bit1; B=bit0	
	if(cable_type == HEADSET_MIC) {//HEADSET_NO_MIC,set default type is 4line accdet
#ifdef ACCDET_PIN_RECOGNIZATION
		show_icon_delay = 0;
...
#endif
		// disable_accdet(); //don't close for key detect
...
static void accdet_eint_func(void)
{
	...
	else 
	{
		...
		*/
		...
		//BEGIN: FR803757 Modified by fangjie, refer to soul45 longfang 
		mt65xx_eint_set_hw_debounce(CUST_EINT_ACCDET_NUM, CUST_EINT_ACCDET_DEBOUNCE_CN);//ACCDET_SHORT_PLUGOUT_DEBOUNCE_CN
		//END: FR803757 Modified by fangjie, refer to soul45 longfang 
#endif        
		...
		micbias_timer.expires = jiffies + MICBIAS_DISABLE_TIMER;//6 Hz
		micbias_timer.function = &disable_micbias;	//turn on accdet_disable_workqueue and then disable_micbias_callback
		micbias_timer.data = ((unsigned long) 0 );
		//add_timer(&micbias_timer);
		micbias_times_flag=1;
		*/
		//END: FR803757 Modified by fangjie, refer to soul45 longfang 
#endif
	}
...
static void send_key_event(int keycode,int flag)
{
	if(call_status == 0)
	{
		...
		case UP_KEY:
			...
			//BEGIN: FR803757 Modified by fangjie, refer to soul45 longfang 
		default:
			ACCDET_DEBUG("No KEY %d\n",flag);
			break;
			//END: FR803757 Modified by fangjie, refer to soul45 longfang d
		}
	}
	else
	{
...
#if 0
	//BEGIN: add by fangjie for PR851790, refer to PR851125
	if(cali_voltage>MIC_VOLTAGE18){//add by llf for PR851125
		mic_press_voltage=MIC_VOLTAGE18;
	}
	//END: add by fangjie for PR851790, refer to PR851125
#endif
/*End   ersen.shang 20150818 remove the modifications reference pr851125 for mid_key issue*/
...
#define PIN_ADC_CHANNEL 5
	...
	ACCDET_DEBUG("[Accdet] switch check_cable_type: ACCDET_STATUS = %d \n", accdet_status);
	IRQ_CLR_FLAG = FALSE;
	//END: FR803757 Modified by fangjie, refer to soul45 longfang 
...
#if 0					
					//BEGIN: add by fangjie for PR851790, refer to PR851125
					if(mic_press_voltage<MIC_VOLTAGE18){//add by llf for PR851125
						notify_sendKeyEvent(ACC_MEDIA_PLAYPAUSE);
					}else{
						mic_press_voltage=0;
						ACCDET_DEBUG("[Accdet] Short press middle between in plug-out!!!! \n");
					}
					//END: add by fangjie for PR851790, refer to PR851125
#else
					notify_sendKeyEvent(ACC_MEDIA_PLAYPAUSE);
...
static int dump_register(void)
{
	...
	{
		printk(" [Audio_Accdet]ACCDET_BASE + 0x%x=0x%x\n",i,pmic_pwrap_read(ACCDET_BASE + i));
	}
	printk(" [Audio_Accdet]ACCDET_STATE_RG =0x%x\n",pmic_pwrap_read(ACCDET_STATE_RG));
	printk(" [Audio_Accdet]ACCDET_STATE_SWCTRL =0x%x\n",pmic_pwrap_read(ACCDET_STATE_SWCTRL));
	printk(" [Audio_Accdet]ACCDET_IRQ_STS =0x%x\n",pmic_pwrap_read(ACCDET_IRQ_STS));
	printk(" [Audio_Accdet]ACCDET_CTRL =0x%x\n",pmic_pwrap_read(ACCDET_CTRL));
	printk(" [Audio_Accdet]0x0400 =0x%x\n",pmic_pwrap_read(0x0400));
	printk(" [Audio_Accdet]INT_CON_ACCDET =0x%x\n",pmic_pwrap_read(INT_CON_ACCDET));//INT register in 6320
	printk(" [Audio_Accdet]INT_STATUS_ACCDET =0x%x\n",pmic_pwrap_read(INT_STATUS_ACCDET));//INT register in 6320
	printk(" [Audio_Accdet]IRQ_STATUS_BIT =0x%x\n",pmic_pwrap_read(IRQ_STATUS_BIT));
	printk(" [Audio_Accdet]TOP_RST_ACCDET =0x%x\n",pmic_pwrap_read(TOP_RST_ACCDET));// reset register in 6320
	printk(" [Audio_Accdet]TOP_CKPDN =0x%x\n",pmic_pwrap_read(TOP_CKPDN));// clock register in 6320
#ifdef ACCDET_PIN_SWAP
