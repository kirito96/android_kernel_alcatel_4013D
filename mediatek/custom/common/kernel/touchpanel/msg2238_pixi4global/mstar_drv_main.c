...
s32 DrvMainTouchDeviceInitialize(void)
{
	...
	//Begin: 	modify by fangjie for PR945812
	//BEGIN: modify by fangjie , change TP0 test node from sysfs to procfs. 
	//Because CTS  not check procfs, so set node propertity from 644 to 666, and "*#2886#" can test TP0.
#ifdef CONFIG_ENABLE_ITO_MP_TEST
	_DrvMainCreateProcfsDirEntry(); // Create procfs directory entry
#endif //CONFIG_ENABLE_ITO_MP_TEST
	//END: modify by fangjie for PR945812 
...
#ifdef CONFIG_ENABLE_ITO_MP_TEST
	#if 0 //fangjie delete for PR945812 
    // test
