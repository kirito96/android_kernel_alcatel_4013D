...
{
	#include "TCL_PIXI3_45_3407_TDT0xe0_Ver0x11_20141127_app.i"
};
...
int fts_ctpm_fw_upgrade_with_i_file(void)
{
	...
	if(tp_vendor == 0xe0){
		...
		else if(TP_VENDOR_ID==FT_TDT_3407)
		{
...
int fts_ctpm_auto_upg(unsigned char tp_vendor)
{
	...
	unsigned char version_list_pixi45_3407_tdt[] = {0x01,0x02,0x10,0x11};  
	unsigned char version_list_camry_truly[] = {0x10};  //Keep the Truly tp firmware old version list that allows to be updated.
	int i,i_ret = -1;
	unsigned char reg_val[2] = {0};
	...
	if(tp_vendor == 0x53){
		...
		{
			if(uc_tp_fm_ver<uc_host_fm_ver ||uc_tp_fm_ver==0x00)
			{
				       ...
				       char *firmware_name)
{
	...
	if ((pbt_buf[fwsize - 8] ^ pbt_buf[fwsize - 6]) == 0xFF
		...
		if(TP_VENDOR_ID==FT_TDT_3407)
		i_ret = fts_3407_ctpm_fw_upgrade(pbt_buf, fwsize);
		else if(TP_VENDOR_ID==FT_TDT_3306)
...
*/
	...
	if((tp_vendor != 0xe0) && (tp_vendor != 0x53) )
	{
