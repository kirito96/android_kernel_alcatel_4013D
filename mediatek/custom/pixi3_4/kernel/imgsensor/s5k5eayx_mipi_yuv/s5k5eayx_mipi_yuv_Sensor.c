...
static void S5K5EAYX_Init_Setting(void)
{
	...
	*/ //delete_AF
	...
	S5K5EAYX_write_cmos_sensor(0x0F12, 0x0288);          //AWB B point                     
	...
	S5K5EAYX_write_cmos_sensor(0x0F12, 0x0080);          //Fls  BRIn                       
	S5K5EAYX_write_cmos_sensor(0x0F12, 0x0100);                                   
	S5K5EAYX_write_cmos_sensor(0x0F12, 0x0130);                                   
	S5K5EAYX_write_cmos_sensor(0x0F12, 0x0020);         // Fls  BROut                     
	...
	S5K5EAYX_write_cmos_sensor(0x0F12, 0x0900);     
	...
	{
		0x0F,  0x12,   
		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0100);     
		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0101);     
		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0101);     
		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0001);     

		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0101);     
		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0201);     
		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0102);     
		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0101);     

		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0101);     
		0x01,  0x06,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0202);     
		0x06,  0x01,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0202);     
		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0101);     

		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0201);     
		0x06,  0x0F,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0302);     
		0x0F,  0x06,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0203);     
		0x00,  0x00,						//S5K5EAYX_write_cmos_sensor(0x0F12, 0x0102);     
	...
	S5K5EAYX_write_cmos_sensor(0x0F12, 0x03E8);  //REG_0TC_CCFG_usMaxFrTimeMsecMult10 10fps                                                               
...
void S5K5EAYXHdr_Part2(UINT16 para)
{
	...
	case AE_EV_COMP_00:
		...
		S5K5EAYX_write_cmos_sensor(0x0028,0x2000);                                              
		S5K5EAYX_write_cmos_sensor(0x002A,0x0274);                                              
		S5K5EAYX_write_cmos_sensor(0x0F12,0x0110);  
		break;
	case AE_EV_COMP_n10:
	case AE_EV_COMP_n20:
		SENSORDB("[5EA]:[S5K5EAYX] HDR AE-20\n");		
		...
		break;
	default:
		SENSORDB("[5EA]:[S5K5EAYX] HDR DEFAULT\n");				
		break; 
	}
...
//#define S5K5EAYX_LOAD_FROM_T_FLASH
#ifdef  S5K5EAYX_LOAD_FROM_T_FLASH
...
}S5K5EAYX_initial_set_struct;

static	S5K5EAYX_initial_set_struct S5K5EAYX_Init_Reg[2000];
static  u8 data_buff[50*1024] ;

#define S5K5EAYX_OP_CODE_INI		0x00		/* Initial value. */
#define S5K5EAYX_OP_CODE_REG		0x01		/* Register */
#define S5K5EAYX_OP_CODE_DLY		0x02		/* Delay */
#define S5K5EAYX_OP_CODE_END		0x03		/* End of initial setting. */
#define TFLASH_FILE_PATH            "/sdcard/s5k5eayx_sd.dat"
...
static u8 S5K5EAYX_Initialize_from_T_Flash()
{
	u8  *curr_ptr = NULL;
	...
	fp = filp_open(TFLASH_FILE_PATH, O_RDONLY , 0); 

	if (IS_ERR(fp)) 
	{ 
		printk("S5K5EAYX create file error\n"); 
		return 2;
	} 
	else
	{
		printk("S5K5EAYX_Initialize_from_T_Flash Open File Success\n");
	}
	...
	printk("S5K5EAYX read file over\n");
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			S5K5EAYX_Init_Reg[i].op_code = S5K5EAYX_OP_CODE_REG;

			S5K5EAYX_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 6;	/* Skip "00, 0x" */

			S5K5EAYX_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		}
		else									                /* DLY */
		{
			...
			curr_ptr += 4;	
			S5K5EAYX_Init_Reg[i].op_code = S5K5EAYX_OP_CODE_DLY;

			S5K5EAYX_Init_Reg[i].init_reg = 0xFF;
			S5K5EAYX_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	printk("S5K5EAYX parser settings over \n");

	S5K5EAYX_Init_Reg[i].op_code = S5K5EAYX_OP_CODE_END;
	S5K5EAYX_Init_Reg[i].init_reg = 0xFFFF;
	S5K5EAYX_Init_Reg[i].init_val = 0xFFFF;
	i++;

	/* Start apply the initial setting to sensor. */
	for (j=0; j<i; j++)
	{
		printk(" 0x%x  ==>  0x%x\n",S5K5EAYX_Init_Reg[j].init_reg, S5K5EAYX_Init_Reg[j].init_val);

		if (S5K5EAYX_Init_Reg[j].op_code == S5K5EAYX_OP_CODE_END)	/* End of the setting. */
		{
			break ;
		}
		else if (S5K5EAYX_Init_Reg[j].op_code == S5K5EAYX_OP_CODE_DLY)
		{
			msleep(S5K5EAYX_Init_Reg[j].init_val);		/* Delay */
		}
		else if (S5K5EAYX_Init_Reg[j].op_code == S5K5EAYX_OP_CODE_REG)
		{
			S5K5EAYX_write_cmos_sensor(S5K5EAYX_Init_Reg[j].init_reg, S5K5EAYX_Init_Reg[j].init_val);
		}
		else
		{
			printk("REG ERROR!\n");
	...
	printk("S5K5EAYX apply settings over \n");
...
UINT32 S5K5EAYXOpen(void)
{
	...
	printk("S5K5EAYX Open File Start\n");
	fp = filp_open(TFLASH_FILE_PATH, O_RDONLY , 0); 

	if (IS_ERR(fp)) 
	{ 
		printk("S5K5EAYX open file error\n");
		S5K5EAYX_Init_Setting();		
	} 
	else 
	{
		printk("S5K5EAYX open file success\n");
		filp_close(fp, NULL); 
		S5K5EAYX_Initialize_from_T_Flash();
	}
#else
	S5K5EAYX_Init_Setting();
#endif
...
void S5K5EAYX_set_iso(UINT16 para)
{
	...
	case AE_ISO_100:
		S5K5EAYX_write_cmos_sensor(0x0028, 0x2000);
		S5K5EAYX_write_cmos_sensor(0x002a, 0x050C);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0001);  //ISO type
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0240);  //ISO value
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0001);  //ISO changed
	...
	case AE_ISO_400:
		...
		S5K5EAYX_write_cmos_sensor(0x002a, 0x050C);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0001);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0310);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0001);
		S5K5EAYX_write_cmos_sensor(0x002a, 0x098E);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0200);
		mdelay(100);
		...
		break;
	case AE_ISO_800:
		SENSORDB("[5EA]:S5K5EAYX S5K5EAYX_set_iso func:800\n");
		S5K5EAYX_write_cmos_sensor(0x0028, 0x2000);
		S5K5EAYX_write_cmos_sensor(0x002a, 0x050C);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0001);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0400);
	...
	case AE_ISO_1600:
		SENSORDB("[5EA]:S5K5EAYX S5K5EAYX_set_iso func:1600\n");		
		S5K5EAYX_write_cmos_sensor(0x0028, 0x2000);
		S5K5EAYX_write_cmos_sensor(0x002a, 0x050C);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0001);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0600);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0001);
		S5K5EAYX_write_cmos_sensor(0x002a, 0x098E);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0200);
		mdelay(100);
		...
		S5K5EAYX_write_cmos_sensor(0x002a, 0x050C);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0001);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0550);  //
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0001);
		S5K5EAYX_write_cmos_sensor(0x002a, 0x098E);
		S5K5EAYX_write_cmos_sensor(0x0F12, 0x0200);
		break;
