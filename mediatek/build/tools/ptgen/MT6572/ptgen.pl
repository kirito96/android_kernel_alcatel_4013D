...
#zjg modify for pr921016
my $ssv_config_file = "device/jrdcom/common/jrdssv.mk" ;
#my $ssv_enable =  &get_ssv_config();
my $ssv_enable = $ENV{BUILD_SSV};
print "ptgen get ssv function : ($ssv_enable)\n";
...
print "ptgen get partition_table  : ($PART_TABLE_FILENAME) \n";
#end zjg modify for pr921016
...
#begin  zjg modify for pr921016
sub get_ssv_config
{
			my $enable = "false" ;
			print "get_ssv_config 1";
			if(-e $ssv_config_file)
			{
					open (FILE_HANDLE, "<$ssv_config_file") or die "Cannot open  __FILE__, __LINE__ \n";
					while (<FILE_HANDLE>)
					{
					  if (/^TARGET_BUILD_SSV\s*.*=(\S+)/)
					  {
						    my $cfg= $1;
						    print "get_ssv_config  value = $cfg \n";
						    if($cfg eq "true")
						    {
						    	$enable = "true" ;
						    }
		...
		print "get_ssv_config  enable ssv is : $enable \n";
		return $enable ;
}
#end  zjg modify for pr921016