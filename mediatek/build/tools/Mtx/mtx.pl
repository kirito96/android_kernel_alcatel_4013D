...
#copy cfg files to out dir
print "the current dir is $cwd\n";
my $mtx_dir = "mediatek/build/tools/Mtx/MTXGen";
my $out_dir = "out/target/product/$task_dir_pro/MTXGen/";
print "mtxgen dir is $mtx_dir\n";
print "the out dir is $out_dir\n";
if (-e $out_dir){
print "test\n";
system("rm -rf out/target/product/$task_dir_pro/MTXGen/");
}
system("cp -r $mtx_dir $out_dir");
...
#my $test=system ("cd $cwd/mediatek/build/tools/Mtx && ./ImageMakerMTK -g --scatter $cwd/out/target/product/$task_dir_pro/$plat_path --platform $task_dir_Pla");
#print "test is $test";
if(system ("cd $cwd/out/target/product/$task_dir_pro/MTXGen && ./ImageMakerMTK -g --scatter $cwd/out/target/product/$task_dir_pro/$plat_path --platform $task_dir_Pla")==0)
{
...
if(system ("cd $cwd/out/target/product/$task_dir_pro/MTXGen && ./ImageMakerMTK -a -f $cwd/out/target/product/$task_dir_pro/MTXGen/config/$task_dir_Pla.cfg.xml")==0)
{
