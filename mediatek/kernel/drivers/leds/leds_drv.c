	...
	if(buf != NULL && size != 0)
		{
			...
			if(notify_led == 1)
				{
					//mt_set_gpio_out(GPIO14,GPIO_OUT_ONE);//add by xiaopu.zhu for charge LED
					notify_led_enable = 1;
			...
			if(notify_led == 0)
				{
					//mt_set_gpio_out(GPIO14,GPIO_OUT_ZERO);//add by xiaopu.zhu for charge LED
					notify_led_enable = 0;
